...
// ── AI 친구 5턴 플로우 (Issue #144 + #146 fix) ──
...
async function handleAiFriendChat(utterance, userKey) {
  ...
  // Issue #146 fix 1: 공백 정규화 후 DANGER_KEYWORDS 감지
  const normalizedUtterance = utterance.replace(/\s+/g, '');
  const normalizedKeywords = DANGER_KEYWORDS.map(k => k.replace(/\s+/g, ''));
  if (normalizedKeywords.some(k => normalizedUtterance.includes(k))) {
  ...
  // Issue #146 fix 2: turns >= 5 (기존 > 5 버그 수정)
  if (session.turns >= 5) {
    aiSessions.delete(userKey);
    return {
      version: '2.0',
