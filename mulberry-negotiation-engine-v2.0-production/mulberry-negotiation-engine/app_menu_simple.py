...
with st.sidebar:
    st.title("🌾 Mulberry")
    st.caption("Community Control Center v1.0.2")
    ...
    selected_menu = st.radio(
        "메뉴 선택",
        ["🏠 홈", "💬 협상 엔진", "📊 Mission Control", "👥 사용자 관리", 
         "📧 Email AI", "🧠 mHC 대시보드", "🔧 설정"],
        index=1,  # 협상 엔진이 기본
        label_visibility="collapsed"
    )
    
    st.session_state.current_menu = selected_menu.split(" ", 1)[1]  # 이모지 제거
...
if st.session_state.current_menu == "홈":
    ...
    st.caption("15분 전")
    st.write("🟡 시스템 업데이트: v1.0.2 배포 완료")
    st.caption("1시간 전")

elif st.session_state.current_menu == "협상 엔진":
    # ==================== 기존 협상 엔진 코드 (그대로 유지) ====================
    # 여기서 기존 app_original.py의 내용을 그대로 포함
    exec(open('app_original.py').read().replace(
        "st.set_page_config(",
        "# st.set_page_config("  # 이미 설정되어 있으므로 주석 처리
    ))
