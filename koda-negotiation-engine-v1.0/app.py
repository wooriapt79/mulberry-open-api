"""
Mulberry Agent Negotiation Engine
Inje-gun Seohwa-myeon Joint Purchase Simulator

Agent-to-Agent Negotiation Demo
"""
...
with tab1:
    ...
    for item_name, item_info in product_items.items():
        with st.expander(
            ...
            # 🔥 안전하게 progress 값 처리 (키가 없으면 현재 수량 기준으로 계산)
            if 'progress' in details:
                progress_value = details['progress']
            else:
...
with tab2:
    ...
    if st.session_state.selected_item:
        ...
        st.info(f"**{item_name}** 협상 프로세스를 시민레이션합니다...")
        supply_agent = st.session_state.agents[item_info['agent_id']]
