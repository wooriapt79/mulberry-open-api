...
with st.sidebar:
    ...
    total_participants = sum(item['current'] for item in product_items.values())
...
with tab1:
    ...
    for item_name, item_info in product_items.items():
        with st.expander(f"📦 {item_name} (현재 {item_info['current']}/{item_info['goal']}명 참여중)", expanded=True):
            ...
            with col1:
                st.metric("현재 협상가", f"₩{negotiated_price:,}", f"-₩{details['savings']:,} ({details['total_discount_rate']}%)", delta_color="inverse")
...
with tab3:
    ...
    col1, col2, col3 = st.columns(3)
    col1.metric("주민 총 절감액", f"₩{impact['total_savings']:,}", f"↑ {impact['savings_rate']}%")
    col2.metric("Mulberry 운영 수익", f"₩{impact['mulberry_revenue']:,}", "정산 예정", delta_color="normal")
    col3.metric("AP2 증명 완료 건수", f"{impact['total_transactions']}건", "Verified", delta_color="off")
    st.divider()
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("💰 비용 분석")
        st.write(f"원래 총 비용: ₩{impact['total_original']:,}")
        st.write(f"협상 후 총 비용: ₩{impact['total_negotiated']:,}")
        st.write(f"총 절감액: ₩{impact['total_savings']:,} ({impact['savings_rate']}%)")
        st.divider()
...
with col2:
    if st.button("🔄 전체 초기화"):
        ...
        st.rerun()
with col3:
    st.caption("버전: v1.0.1")
    st.caption("지역: 인제군 서화면")
